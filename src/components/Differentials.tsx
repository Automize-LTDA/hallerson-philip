...
export default function Differentials() {
  ...
  const diffs = [
    {
      icon: <Sparkles className="text-brand-red" size={24} />,
    ...
    {
      icon: <Trophy className="text-brand-red" size={24} />,
    ...
    {
      icon: <Sliders className="text-brand-red" size={24} />,
  ...
  return (
    <section id="diferenciais" className="relative py-12 md:py-16 bg-brand-black border-t border-brand-gray-dark/30 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-red/3 rounded-full blur-[160px] pointer-events-none"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {diffs.map((item, index) => (
            <motion.div
              ...
              className="p-8 rounded-xl bg-brand-graphite/60 border border-brand-gray-dark/50 hover:border-brand-red/40 transition-all duration-300 group safari-anim-fix"
            >
              {/* Icon Container with subtle glow */}
              <div className="w-12 h-12 rounded-lg bg-brand-red/5 border border-brand-red/10 flex items-center justify-center mb-6 group-hover:bg-brand-red/10 group-hover:border-brand-red/30 transition-colors duration-300">
                {item.icon}
              </div>

              {/* Title */}
              <h3 className="text-white font-display font-bold text-base uppercase tracking-wider mb-3 group-hover:text-brand-red transition-colors duration-300">
                {item.title}
              </h3>
